...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss_each = -np.log(prob[np.arange(N), y])
    loss = np.sum(loss_each) / N
    ...
    db2 = np.sum(dscores, axis=0)  # 1 * C

    dh1 = np.dot(dscores, W2.T)  # N * H
    dh1[h1 <= 0] = 0  # N * H

    dW1 = np.dot(X.T, dh1)  # D * H
    db1 = np.sum(dh1, axis=0)  # 1 * H
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      sample_indexs = np.random.choice(num_train, batch_size, replace=True)
